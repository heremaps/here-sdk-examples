...
1. Add your HERE SDK credentials to
   `HERESDKUnitsKotlin/app/src/main/java/com/here/heresdkunitskotlin/MainActivity.kt`.
...
4. Optionally, add a res folder under `HERESDKUnits/<your-module>/src/main/res/`.
...
Add a `YXUnit` class for the logic. And a `XYView` class for the UI / Composables of your your unit.

To start from scratch create the view:
...
fun HelloView(
    ...
    Text(text = "Hello Units: ${unit.uiState.counter}")
}
```

And the unit to handle the business logic and UI state:

```kotlin
// Make sure to have these imports, or mutableStateOf will fail to compile.
...
class HelloUnit {
    internal var uiState by mutableStateOf(HelloUiState())
        private set

    fun setCounter(value: Int) {
        uiState = uiState.copy(counter = value)
    }
}
```
...
And add HelloView for testing to the `MainActivity` in the app module. But before testing the unit make sure to configure the module as described below.

Configure your new module:

- Add a compile-time reference to the HERE SDK AAR (units should not bundle the HERE SDK) in  the unit module’s `build.gradle` and add also the compose dependencies:
...
- Add the plugin for compose and add a version:
...
Add also the compose build features to the `android` block:
...
- Set Java compatibility, if not done already:
...
and append the version and a kotlin tag **manually**, like this: `here-sdk-units-core-kotlin-v1.0.aar`
