...
- **WarnerEngine (Beta)**: The new unified approach using a single WarnerListener on the WarnerEngine obtained from the VisualNavigator. All warning events are received through one callback and detailed warning data is looked up via the WarningsRegistry.
