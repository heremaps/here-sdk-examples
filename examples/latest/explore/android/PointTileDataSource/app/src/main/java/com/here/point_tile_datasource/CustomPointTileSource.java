...
public class CustomPointTileSource implements PointTileSource {
    ...
    @Nullable
    @Override
    public LoadTileRequestHandle loadTile(
            @NonNull TileKey tileKey, @NonNull LoadResultHandler loadResultHandler) {
        if (mHasData.get()) {
            ...
            PointData tileData = new PointDataBuilder()
                    .withCoordinates(new GeoCoordinates(
                            52.56726841787437, 13.285501201751865))
                    .withAttributes(pointAttributes)
                    .build();
            loadResultHandler.loaded(
                    tileKey, Arrays.asList(tileData), new TileMetadata(mDataVersion, new Date(0)));
        } else {
            loadResultHandler.loaded(
                    tileKey,
                    new ArrayList<>(),
    ...
    public void setHasData(boolean hasData) {
        ...
        if (mListener != null) {
            mListener.onDataVersionChanged(mHasData.get() ? mDataVersion : mDataVersionWhenEmpty);
        }
    }
}
